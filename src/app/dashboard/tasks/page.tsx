...
import { RefreshCw, CheckCircle, Clock, AlertTriangle, CheckSquare, ExternalLink, XCircle, Minus } from 'lucide-react'
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-wrap items-start justify-between gap-4">
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          <Button variant="tertiary" size="icon" onClick={fetchTasks} disabled={loading} aria-label="Refresh tasks">
      ...
      <div className="surface-card flex flex-wrap items-center justify-between gap-x-6 gap-y-3 px-4 py-3">
        <p className="text-[13px] text-white/50">
          Tasks awaiting sign-off are only visible inside Trail — the public API does not expose approval status.
        </p>
        <a
          href="https://web.trailapp.com/reports"
          target="_blank"
          rel="noopener noreferrer"
          className="inline-flex h-8 shrink-0 items-center gap-1.5 rounded-[10px] bg-white/[0.06] px-3 text-[13px] font-medium text-white/70 transition-colors hover:bg-white/[0.09] hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-ring/40"
        >
          Pending approvals in Trail <ExternalLink className="size-3.5" />
        </a>
      </div>
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        {[
          ...
          { label: 'Done',       value: doneCount,      colour: doneCount > 0 ? 'text-emerald-300' : 'text-white/60' },
      ...
      <div role="tablist" aria-label="Filter tasks" className="inline-flex flex-wrap rounded-xl bg-white/[0.06] p-1">
        {(['all', 'done', 'inprogress', 'overdue', 'missed'] as const).map(f => {
          const count = f === 'overdue' ? overdueCount : f === 'missed' ? missedCount : null
          return (
            <button
              key={f}
              role="tab"
            ...
            >
              {FILTER_LABELS[f]}
              {count != null && count > 0 && (
                <span className={`flex h-4 min-w-4 items-center justify-center rounded-full px-1 text-[10px] font-semibold ${f === 'overdue' ? 'bg-red-500/20 text-red-300' : 'bg-white/10 text-white/50'}`}>
                  {count}
                </span>
              )}
            </button>
